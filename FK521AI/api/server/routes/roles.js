...
function normalizeRoleName(rawRoleName) {
  ...
  try {
    return decodeURIComponent(rawRoleName).trim().toUpperCase();
  } catch {
    return rawRoleName.trim().toUpperCase();
  }
}
...
const createPermissionUpdateHandler = (permissionKey) => {
  ...
  return async (req, res) => {
    ...
    try {
      ...
      if (!error?.errors) {
        logger.error(`[PUT /roles/:roleName/${permissionKey}] Error:`, error);
        return res.status(500).send({ message: 'Failed to update role permissions' });
      }
